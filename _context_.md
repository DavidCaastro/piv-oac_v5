...
| **B1.1** — `_parse_verdict()` default REJECTED (regresión de seguridad v4) | 2026-04-21 s9 | En v5 el default era `True` (APPROVED) cuando el LLM no declaraba `GATE_VERDICT` explícito. Revertido a `False` (REJECTED) — ambigüedad = fallo de seguridad, paridad con v4. Agregada detección de `APPROVED` standalone (simétrico al `REJECTED` ya existente) antes de caer al default. Afecta PHASE 3, PHASE 7. |
| **B1.2** — Retry + timeout en `AsyncAnthropicProvider` y `AsyncOllamaProvider` | 2026-04-21 s9 | `complete()` llamaba al SDK sin timeout ni retry — `asyncio.gather()` en PHASE 5 podía quedar colgado indefinidamente. Agregadas constantes `_TIMEOUT_S=120`, `_MAX_RETRIES=3`, `_BASE_DELAY_S=1.0`, `_RETRYABLE=(429,500,502,503,529)`. Loop `for attempt in range(_MAX_RETRIES)` + `asyncio.wait_for()`. Backoff exponencial `1.0 * 2^attempt`. Anthropic: retry en `TimeoutError`, `RateLimitError`, `APIStatusError` retryable. Ollama: retry en `ConnectError` y `RemoteProtocolError`. |
| **B1.3** — TTL en PMIABroker (`_MESSAGE_TTL_MS = 30_000`) | 2026-04-21 s9 | `PMIAMessage.timestamp_ms` existía pero nunca se validaba — mensajes capturados podían reenviarse indefinidamente (replay attack). Agregada validación en `send()` entre `validate_size()` y `_sign()`: age > 30s → `PMIAError("expired")`; timestamp futuro > 5s → `PMIAError("future timestamp")`. `import time` añadido a `broker.py`. |
| **B2.1** — `sdk/exceptions.py` — jerarquía tipada de excepciones (paridad v4) | 2026-04-21 s9 | Archivo nuevo. 8 clases: `PIVOACError` (base), `AgentUnrecoverable`, `GateRejected(gate, rationale)`, `VetoError`, `MalformedOutput(agent_id, expected, got)`, `CircuitOpen(threshold, agent_id)`, `MessageExpired`, `MessageTampered`. Exportadas desde `sdk/__init__.py`. Las excepciones existentes en sus módulos no se modificaron — jerarquía nueva para adopción incremental. |
| **B2.2** — `sdk/core/contract_parser.py` — `ContractParser` + `EvalScoreResult` | 2026-04-21 s9 | Archivo nuevo. Centraliza toda la lógica de parseo LLM que estaba dispersa en `session_async.py`. Todos los métodos `@staticmethod` (sin estado mutable). `parse_verdict()`: default `False` (security invariant). `extract_rationale()`: primeros 200 chars. `parse_eval_scores()`: (1) fence `json`, (2) fallback `_JSON_RAW_RE` para JSON sin fence (fix Task 41), (3) `EvalScoreResult(parse_error=True)` si nada. `MalformedOutput` solo si fence existe pero JSON inválido. `EvalScoreResult` dataclass con `to_dict()`. Las 3 funciones módulo-nivel eliminadas de `session_async.py`; 4 call-sites migrados. |
| **B2.3** — `sdk/agents/base.py` — `AgentBase.call()` + `AgentCallResult` | 2026-04-21 s9 | Directorio `sdk/agents/` nuevo (`__init__.py` + `base.py`). `AgentBase.call(provider, request, agent_id, session_id, timeout_s=120, max_retries=2)` aplica retry+backoff a `TimeoutError` y `ProviderError`; cualquier otro error → `AgentUnrecoverable` inmediato. `AgentCallResult` dataclass: `content`, `input_tokens`, `output_tokens`, `model`, `attempts`, `duration_ms`. Aplicado en `_run_specialist()` — el path crítico de `asyncio.gather()`. Capa complementaria a B1.2 (provider retry = errores de red; AgentBase retry = errores de agente). |
| **B3.1** — PHASE 4 — `_run_domain_orchestrator()` wired en `session_async.py` | 2026-04-21 s9 | Task 40 cerrado. Método `_run_domain_orchestrator(session_id, dag, objective, sec_rationale)` implementado: carga `domain_orchestrator` config, LLM call BALANCED, parsea con `ContractParser`, emite `gate_verdict(GATE_2)` + `checkpoint_req(PHASE_4)`. `gate_verdicts["GATE_2"]` en `AsyncSessionResult`. Fallo de config/LLM → `(True, "", "skipped")` no-fatal. Inserción en `run_async()` entre PHASE 3 y PHASE 5. `sec_rationale` elevado a scope externo con default `""`. |
| **B3.2** — `engram/bias/` persistencia en `_run_bias_audit()` | 2026-04-21 s9 | Task 42 cerrado. `_run_bias_audit()` ahora escribe `engram/bias/<session_id>/audit.md` después del checkpoint. Incluye: objective, verdict (PASSED/REJECTED), red_team_result, multi_llm_result, missing_sections, output_text[:4000]. Best-effort: wrapped en `try/except` — no bloquea pipeline. `engram/bias/` se puebla en cada sesión L2. |
| **B3.3** — `_parse_eval_scores()` fallback robusto (Task 41) | 2026-04-21 s9 | Cubierto por B2.2. `ContractParser._JSON_RAW_RE` captura JSON sin fence con key "scores". Antes: cualquier respuesta LLM sin bloque ```json``` resultaba en `parse_error=True`. Ahora: regex secundario intenta extraer el objeto JSON crudo antes de rendirse. |
| **B3.4** — Integration tests PHASE 3, 6, 7 (Task 43) | 2026-04-21 s9 | 4 archivos creados en `tests/integration/`: `conftest.py` (fixtures: `make_stub_dag`, `mock_provider_response`, `make_mock_session`), `test_phase3_security.py` (9 casos), `test_phase6_evaluation.py` (6 casos), `test_phase7_coherence.py` (3 casos). Todos `@pytest.mark.integration`, `AsyncMock` providers — sin LLM real. 18/18 PASS. Unit tests: 31/31 sin regresión. Total: 49/49. |
...
### Impact Analysis — Session 9 (2026-04-21)

| Implemented | Runtime impact |
|---|---|
| **B1.1** — `_parse_verdict()` default REJECTED | Gates de seguridad (PHASE 3) y coherencia (PHASE 7) ya no aprueban silenciosamente cuando el LLM produce output ambiguo o malformado. Antes: un LLM que respondía sin `GATE_VERDICT` explícito (respuestas largas, off-format) pasaba automáticamente. Ahora: cualquier ambigüedad = REJECTED. Paridad con v4. |
| **B1.2** — Retry + timeout en providers async | `asyncio.gather()` en PHASE 5 ya no queda colgado indefinidamente ante errores de red o rate-limiting. 3 intentos con backoff 1/2/4s antes de `ProviderError`. Resiliente a errores 429/503 transitorios de Anthropic y a desconexiones de Ollama. |
| **B1.3** — TTL 30s en PMIABroker | Mensajes PMIA capturados no pueden reenviarse fuera de ventana. Validación activa en cada `send()` — doble línea de defensa junto a HMAC. Timestamps futuros (reloj adelantado > 5s) también rechazados. |
| **B2.1** — `sdk/exceptions.py` jerarquía tipada | Handlers de excepción pueden ahora hacer `except AgentUnrecoverable` en lugar de `except Exception`. `GateRejected(gate, rationale)` y `MalformedOutput(agent_id, expected, got)` permiten logging estructurado en telemetría sin parseo de strings. `CircuitOpen`, `VetoError`, `MessageExpired`, `MessageTampered` cubren escenarios de seguridad que antes lanzaban `Exception` genérica. |
| **B2.2** — `ContractParser` + `EvalScoreResult` | Lógica de parseo LLM centralizada en un único módulo stateless. Testeable de forma aislada (sin instanciar AsyncSession). `_JSON_RAW_RE` fallback cierra la regresión Task 41: EvaluationAgent ya no retorna `parse_error=True` cuando el LLM omite el fence de código. `MalformedOutput` tipado reemplaza el `pass` silencioso anterior. |
| **B2.3** — `AgentBase.call()` en `_run_specialist()` | El path crítico de `asyncio.gather()` (PHASE 5) tiene ahora retry a nivel agente independiente del retry de provider. `AgentCallResult.attempts` es auditado en telemetría. `AgentUnrecoverable` diferencia fallos de negocio (no retryable) de fallos de red (retryable). Capa de defensa en profundidad: provider retry → errores de red; AgentBase retry → errores de agente. |
| **B3.1** — PHASE 4 DomainOrchestrator | Pipeline L2 completo: PHASE 1.5 → PHASE 3 → PHASE 4 → PHASE 5. DomainOrchestrator evalúa la descomposición de dominios antes de lanzar expertos paralelos. `gate_verdicts["GATE_2"]` propagado al resultado de sesión. `engram/` registra checkpoint `PHASE_4`. |
| **B3.2** — `engram/bias/` poblado | `engram/bias/<session_id>/audit.md` se crea en cada sesión L2. Contiene veredicto, red_team_result, multi_llm_result y primeros 4000 chars del output. Permite análisis histórico de calidad de bias audits entre sesiones. Antes: directory siempre vacío. |
| **Tests 49/49** — suite completa | 18 integration tests con `AsyncMock` providers (sin costo LLM) cubren los 3 phases más críticos: Gate 0 (security invariant B1.1), scoring con/sin fence (fix Task 41), y comportamiento advisory vs blocking de CoherenceAgent. Sin regresiones en los 31 unit tests existentes. |
...
| ~~40~~ | ~~PHASE 4 — DomainOrchestrator (sub-agent routing para dominios especializados)~~ | ~~MED~~ | DONE s9 — `_run_domain_orchestrator()` wired entre PHASE 3 y PHASE 5. `gate_verdicts["GATE_2"]` en AsyncSessionResult. |
| ~~41~~ | ~~EvaluationAgent `_parse_eval_scores()` — mejorar parsing LLM~~ | ~~LOW~~ | DONE s9 — cubierto por `ContractParser._JSON_RAW_RE`. JSON sin fence ahora capturado por regex secundario antes de `parse_error=True`. |
| ~~42~~ | ~~`engram/bias/` — persistir output BiasAuditAgent~~ | ~~LOW~~ | DONE s9 — `_run_bias_audit()` escribe `engram/bias/<session_id>/audit.md` después del checkpoint. Best-effort. |
| ~~43~~ | ~~Tests de integración para PHASE 3, 6, 7~~ | ~~MED~~ | DONE s9 — 18 integration tests creados (conftest + test_phase3/6/7). 49/49 PASS total. |
| ~~44~~ | ~~`_parse_verdict()` default REJECTED — regresión de seguridad v4~~ | ~~HIGH~~ | DONE s9 — default cambiado de `True` a `False`. Ambigüedad = REJECTED (security invariant). |
| ~~45~~ | ~~Retry + timeout en providers async (AsyncAnthropicProvider, AsyncOllamaProvider)~~ | ~~HIGH~~ | DONE s9 — 3 intentos, backoff exponencial, `asyncio.wait_for()` 120s por intento. |
| ~~46~~ | ~~TTL de 30s en PMIABroker — prevención de replay attack~~ | ~~MED~~ | DONE s9 — validación en `send()` entre `validate_size()` y `_sign()`. |
| ~~47~~ | ~~`sdk/exceptions.py` — jerarquía tipada de excepciones (paridad v4)~~ | ~~MED~~ | DONE s9 — 8 clases, exportadas desde `sdk/__init__.py`. |
| ~~48~~ | ~~`sdk/core/contract_parser.py` — `ContractParser` + `EvalScoreResult`~~ | ~~MED~~ | DONE s9 — migración completa de helpers módulo-nivel; fallback JSON sin fence. |
| ~~49~~ | ~~`sdk/agents/base.py` — `AgentBase.call()` wrapper~~ | ~~MED~~ | DONE s9 — retry+timeout a nivel agente; aplicado en `_run_specialist()`. |

> **Pipeline PHASE 0→8 completo y estabilizado como de session 9.** Todas las regresiones v4→v5 corregidas. PHASE 4 (DomainOrchestrator) wired. `engram/bias/` persiste output BiasAuditAgent. EvaluationAgent con fallback robusto. Jerarquía de excepciones tipadas, ContractParser centralizado, AgentBase retry layer operativos. 49 tests (31 unit + 18 integration) en verde. Framework apto para uso con objetivos L1 (fast-track) y L2 (full pipeline).
